"""Regression tests for the 3 RISKY simplify-swarm findings (Phase 4).
...
from blobstore import BlobStore, _SLICE_MASK_MARKER, _LABEL_UPGRADE_PATTERNS
...
class TestGrepMaskBlindWindow:
    ...
    def test_grep_masks_credential_past_char_500(self, tmp_path):
        ...
        # 510 padding + secret = 529-char line (well under 2000 line_cap).
        # Use a grep pattern that ONLY matches at the secret's position
        # (so the match is unambiguous), and the secret sits past char 500.
        line0 = ("a" * 510) + secret
        line1 = "benign trailing line"
        lines = [line0, line1]
        # Grep for the credential itself — the only matching line is
        # line0 (line1 is benign and doesn't contain 'sk-').
    ...
    def test_grep_masks_credential_at_char_100_too(self, tmp_path):
        ...
        credential appears in the first 500 chars (this case already
        works because the truncated body contains the pattern)."""
        store = self._store_with_enforcement(tmp_path)
    ...
    def test_grep_passes_through_benign_long_lines(self, tmp_path):
        ...
        shape must still be returned in full (up to 500-char body
        per the grep_safe cap)."""
        store = self._store_with_enforcement(tmp_path)
        # 600-char benign line — no credential shape, no sk-, no
        # bearer, no BEGIN PRIVATE KEY.
        line0 = "x" * 600
        ...
        # The short matched line must be present verbatim.
    ...
    def test_grep_mask_consistent_with_range_mode(self, tmp_path):
        ...
        guarantee for the same line — the two surfaces should not
        disagree on what is a credential line."""
        store = self._store_with_enforcement(tmp_path)
        secret = "sk-ABCDEFGHIJKLMNOP"
        line0 = ("a" * 510) + secret
        line1 = "benign trailing line"
        lines = [line0, line1]
        # Range mode (which already scans [:2000]):
        range_masked = store._mask_lines(lines)
        # Grep mode (currently scans only [:500] of the truncated body):
        grep_out, _ = store._grep_with_mask(
            lines, "sk-", 4000, mask_lines=True)
        # Both must agree: line 0 is masked, line 1 is not.
    ...
    def test_enforcement_off_grep_unaffected(self, tmp_path):
        ...
        not touch the mask_lines=False branch. With mask_lines=False,
        _grep_with_mask delegates straight to _grep_safe and the
        500-char body truncation (existing _grep_safe behaviour)
        continues to hide the secret at position 510 — the fix must
        ...
        # The OFF path never inserts a mask marker — that is the
        # byte-identical guarantee.
        assert _SLICE_MASK_MARKER not in out, (
            f"enforcement OFF must never produce the mask marker; "
            f"got: {out!r}"
        )
        # The OFF path delegates to _grep_safe which truncates the
        # body to 500 chars, so the credential at position 510 is
        # already excluded from the output. The fix must keep this
        # truncation behaviour verbatim.
        assert "0: " + secret not in out
...
class TestSweepBudgetResetOnNoOp:
    ...
    entry actually transitions to a tombstone. Currently the counter
    is popped BEFORE the TTL-expiry check, so every sweep — including
    sweeps that expire nothing — wipes live budgets. Combined with
    lazy_sweep running on session start AND end, the budget is reset
    twice per session."""
    ...
    def test_no_op_sweep_preserves_credential_served_chars(
        ...
        # Stamp a known counter value via direct index mutation.
        idx = store._load_idx("s1")
        idx["blobs"][bid]["credential_served_chars"] = 1234
        store._save_idx(idx, "s1")
        # Run a sweep. The entry is fresh (t = now), so the TTL
        # check should NOT transition it. The counter MUST survive.
        store.lazy_sweep()
    ...
    def test_repeated_no_op_sweeps_preserve_counter(self, tmp_path):
        """lazy_sweep runs at session start AND end. Two no-op
        sweeps in a row must leave the counter intact (previously
        the counter was reset twice, which the budget contract
        forbids)."""
        store = BlobStore({
            "enforcement_enabled": True,
            "store_path": str(tmp_path / "store"),
    ...
    def test_tombstone_transition_drops_counter(self, tmp_path):
        """Counter-must-vanish-on-tombstone contract is preserved:
        when the entry DOES transition to a tombstone, the counter
        must not survive into the tombstone dict."""
        store = BlobStore({
            "enforcement_enabled": True,
            "store_path": str(tmp_path / "store"),
            "fetch_max_chars": 4000,
    ...
    def test_non_credential_label_unaffected(self, tmp_path):
        """The pop path was gated by label == 'credential'. Verify
        a public blob's index entry is completely untouched by
        ...
        # Add a stray credential_served_chars on a public blob (would
        # be a config bug, but prove sweep doesn't touch public entries).
        idx = store._load_idx("s1")
        ...
        # Public blob untouched (sweep never touched the counter).
        assert entry.get("credential_served_chars") == 42


# ════════════════════════════════════════════════════════════════════════
# RISKY-3 — HIGH — Cross-process lost updates on budget counter
# ════════════════════════════════════════════════════════════════════════


class TestChargeBudgetCrossProcessSafe:
    """_charge_slice_budget must serialise its read-modify-write
    across processes, not just threads. The in-process threading.Lock
    only synchronises threads in the SAME process; two processes
    charging concurrently against the same store path would lose
    increments. The fix: fcntl.flock a dedicated <store_path>/.budget.lock
    file around the RMW. Two in-process instances charging through
    interleaved fresh-read-modify-write cycles both land."""
    ...
    def test_two_instances_interleaved_charges_land(self, tmp_path):
        ...
        bid = s1.put("payload", "send_email", session_id="sA",
                     label="credential")
        # Charge 100 from instance s1.
        s1._charge_slice_budget(bid, "sA", 100, mask_slices=True)
        # Instance s2 must re-read from disk (not see s1's in-memory
        # state) and charge 100 on top. Without flock, both reads
        # could see 0 and one write would clobber the other.
        s2._charge_slice_budget(bid, "sA", 100, mask_slices=True)
        # Read back through a fresh instance to bypass any cache.
    ...
    def test_lockfile_created_and_released(self, tmp_path):
        ...
        created when the store is initialised, and released after
        _charge_slice_budget returns (so a second process can
        acquire it)."""
        store = self._store(tmp_path)
        bid = store.put("payload", "send_email", session_id="sA",
                        label="credential")
        lock_path = Path(store.store_path) / ".budget.lock"
        # The lockfile must exist after store init (so two processes
        # can race on it). It MAY be open by this process — that's
        # fine; what matters is that another process can open it.
        assert lock_path.exists(), (
            f"expected .budget.lock at {lock_path} after store init"
        )
        # After the charge completes, the fd is released. A second
        # charge call must succeed without deadlock.
    ...
    def test_lockfile_is_a_lockfile(self, tmp_path):
        ...
        store.put("payload", "send_email", session_id="sA",
                  label="credential")
        expected = Path(store.store_path) / ".budget.lock"
    ...
    def test_charge_serialised_under_three_callers(self, tmp_path):
        ...
        land (no clobbering). This is a stronger cross-process-style
        witness."""
        s1 = self._store(tmp_path)
        s2 = self._store(tmp_path)
        bid = s1.put("payload", "send_email", session_id="sA",
        ...
        s3 = self._store(tmp_path)
        idx = s3._load_idx("sA")
        # 15 charges of varying size, each adding its share.
        total = idx["blobs"][bid].get("credential_served_chars", 0)
        # s1 added 10+30 = 40 per round, s2 added 20 per round, 5 rounds.
    ...
    def test_no_fcntl_platform_skips_flock_cleanly(self, tmp_path,
                                                     monkeypatch):
        """On platforms without fcntl (e.g. Windows native), the
        charge must still work — fall back to in-process _LOCK and
        log DEBUG. We simulate by patching the module-level
        _HAVE_FCNTL flag the production code checks."""
        from blobstore import _HAVE_FCNTL
        # Force the "no fcntl" branch by flipping the module flag.
        monkeypatch.setattr(_blobstore_mod, "_HAVE_FCNTL", False)
        store = self._store(tmp_path)
        bid = store.put("payload", "send_email", session_id="sA",
                        label="credential")
        # Must not raise even with fcntl marked unavailable.
        store._charge_slice_budget(bid, "sA", 50, mask_slices=True)
        idx = store._load_idx("sA")
...
class TestChargeBudgetCoercionAndFallback:
    ...
    def test_non_int_counter_treated_as_zero_with_warning(self, tmp_path,
        ...
        bid = store.put("payload", "send_email", session_id="sA",
                        label="credential")
        idx = store._load_idx("sA")
        # Inject a string counter — simulates a bad operator edit or
        # a yaml-quirk (the type-eagerness coerces quoted numbers to
        # strings if the loader gets confused).
        ...
        # The new total is 0 + 100 = 100, and a WARNING was logged.
    ...
    def test_persist_failure_logs_warning_and_uses_inmem_fallback(
        ...
        bid = store.put("payload", "send_email", session_id="sA",
                        label="credential")
        # Force _write_idx_file to raise. The in-memory fallback
        # must absorb this so budgeting survives persistence loss.
        from blobstore import _SliceBudgetExceeded
        calls = {"n": 0}
        real_write = store._write_idx_file
        def boom(path, idx):
            calls["n"] += 1
            raise OSError("simulated disk full")
        monkeypatch.setattr(store, "_write_idx_file", boom)
        with caplog.at_level(logging.WARNING,
                              logger="blobstore"):
            # First charge: persist fails, in-memory accumulates 100.
            store._charge_slice_budget(bid, "sA", 100, mask_slices=True)
            # Second charge: persist fails again, in-mem = 200.
            store._charge_slice_budget(bid, "sA", 100, mask_slices=True)
        # In-memory fallback should reflect 200.
        ...
        # WARNING level (not DEBUG) emitted for the persist failure.
        warnings = [r for r in caplog.records
...
class TestChargeBudgetEmptySessionFallback:
    ...
    def test_empty_session_id_uses_global_index(self, tmp_path):
        ...
        '_global' session index, not be silently dropped."""
        store = self._store(tmp_path)
        bid = store.put("payload", "send_email", session_id="real",
                        label="credential")
        # Register the blob in the '_global' index too so the charge
        # has a valid entry to update.
        store._charge_slice_budget(bid, "_global", 100, mask_slices=True)
        idx = store._load_idx("_global")
        assert bid in idx["blobs"]
        assert idx["blobs"][bid].get("credential_served_chars") == 100, (
            f"empty-session charge must land in _global index; "
    ...
    def test_empty_session_id_fallback_logged_once(self, tmp_path,
        ...
        # Must be at most one log line (the once-per-process notice).
